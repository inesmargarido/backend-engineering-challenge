...
def calculate_moving_averages(events, window_minutes):
  ...
  result = {} # dictionary to save result
  ...
  while current_minute <= end_time:
    ...
    # save timestamp + average in dictionary
    result[current_minute] = avg_per_min
...
def main():
  try:
    ...
    output = calculate_moving_averages(events, args.window_minutes)

    # transform the output into a json file in the desired format
    output_json = [
      json.dumps({"date": str(current_minute), "average_delivery_time": avg_per_min}) + '\n'
      for current_minute, avg_per_min in output.items()
    ]

    # write on the file
    with open('output.json','w') as file:
      file.writelines(output_json)
      print(f"The output file was saved: output.json")
